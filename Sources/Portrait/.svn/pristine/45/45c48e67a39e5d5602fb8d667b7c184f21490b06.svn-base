		...
		<c:forEach items="${requestScope.photoList }" var="photo" varStatus="status">
			<c:choose>
				<c:when test="${status.index == 0 || status.index % 5 == 0 }">
					<tr>
						<td><form action="delete.do" method="post" ><img src="${photo.photoUrl }" width="150px"><br>${photo.photoTitle }<br><input type="hidden" name="photoId" value="${photo.photoId }"><input type="hidden" name="action" value="delete"><input type="submit" value="삭제"></form></td>
				</c:when>
				<c:otherwise>
						<td><form action="delete.do" method="post" ><img src="${photo.photoUrl }" width="150px"><br>${photo.photoTitle }<br><input type="hidden" name="photoId" value="${photo.photoId }"><input type="hidden" name="action" value="delete"><input type="submit" value="삭제"></form></td>
				</c:otherwise>
