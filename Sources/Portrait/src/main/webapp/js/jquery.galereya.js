...
var peopleN=$('#numOfPeople').val();
for (var i = 0; i < peopleN; i++) {
	var imgId = '.people' + i;
	var peopleW = $(imgId).attr('data-width');
	peopleW *= 150;
	$(imgId).css('width',peopleW);
}
...
(function ($) {
    ...
    function Galereya(options) {
        ...
        var placeCell = function (cell, number) {
            ...
            var className = '.people' + number;
        	var divWidth = $(className).attr('data-width');
        	
        	divWidth *= 150;
        	
        	console.log(divWidth);
            
            cell.style.width = divWidth + 'px';
        };
